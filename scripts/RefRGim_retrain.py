...
net_path =  sys.argv[3] + "/1KGP_CNN_net"
files= os.listdir(net_path)
chr = {}
for file in files:
    if re.match('chr', file):
        chr[file.replace("chr", "")] = 1

pos = {}
with bz2.open(hgimp_path + "/SNP.INFO.bz2" , 'rt') as f:
    for line in f:
        geno = line.strip('\n').split('\t')
        if geno[0] in chr.keys():
...
print("==>Search for comparable pre-training network...")
...
print("Chr"+str(form_interv[0])+":"+str(form_interv[1])+"_net is chosen. The number of intersection of SNPs between study and reference in this region is "+str(len(study_geno))+"/4000\n")
